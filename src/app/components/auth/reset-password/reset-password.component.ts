  ...
  ngOnInit(): void {
    ...
    if (!this.token) {
      alertMethod(
        ...
        'Oops algo sucedio! Necesitas un token, tal vez ya utilizaste este link',
        'error'
      );
  ...
  resetPassword(): void {
    ...
    this.userService.resetPassword(this.token, password).subscribe({
      ...
      error: (error) => {
        ...
        this.alertComponent.showAlert(
          'Hubo un error al actualizar la contraseña. Por favor, inténtalo nuevamente.',
          'error'
        );
      },
    });
