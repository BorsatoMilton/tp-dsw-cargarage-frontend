...
export class BrandComponent implements OnInit {
  ...
  checkBrandExists(
    ...
    return this.brandService
      .getOneBrandByName(nombreMarca, excludeBrandId)
      .pipe(map((marca: Brand) => !!marca));
  }

  addBrand() {
    if (this.brandForm.valid) {
      const brandData = this.brandForm.value;
      this.checkBrandExists(brandData.nombreMarca)
        .pipe(
  ...
  editBrand(): void {
    if (this.selectedBrand) {
      ...
      this.checkBrandExists(updatedBrand.nombreMarca, this.selectedBrand.id)
        .pipe(
          switchMap((exists: boolean) => {
            if (exists) {
              return throwError(() => new Error('La marca ya existe'));
