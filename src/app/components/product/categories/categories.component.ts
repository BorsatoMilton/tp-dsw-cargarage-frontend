...
export class CategoriesComponent implements OnInit {
  ...
  checkCategoryExists(
    ...
    return this.categoriesService
      .getOneCategoryByName(nombreCategoria, excludeCategoryId)
      .pipe(map((categoria: Category) => !!categoria));
  }
  ...
  addCategory() {
    if (this.categoryForm.valid) {
      ...
      this.checkCategoryExists(categoryData.nombreCategoria)
        .pipe(
  ...
  editCategory(): void {
    if (this.selectedCategory) {
      ...
      this.checkCategoryExists(
        updatedCategory.nombreCategoria,
        ...
        .pipe(
          switchMap((exists: boolean) => {
            if (exists) {
              return throwError(
                () => new Error('El nombre de la categoría ya existe')
              );
            }
        ...
        .subscribe({
          ...
          error: (err: any) => {
            if (err.message === 'El nombre de la categoría ya existe') {
              ...
              this.closeModal('editCategoria');
              this.alertComponent.showAlert(
                'Ocurrió un error al editar la categoría',
                'error'
              );
            }
            this.categoryForm.reset();
